...
def main(args):
	...
	for example_name in examples_list:
		...
		print('\n -------- starting ASAP WITH RESOURCE CONSTRAINTS scheduling -------- \n')
		...
		print('\n -------- starting ASAP PIPELINED scheduling -------- \n')
		...
		print('\n -------- starting ASAP PIPELINED RESOURCE CONSTRAINED scheduling -------- \n')
		...
		while(not resources.add_resource_constraints_pipelined(ilp, constraints, opt_function, budget = 30)): # loop until heuristics finds a feasible solution
			II += 1
			print('II = ', II)
			scheduler.set_II_constraints(II) # replace old constraints with new ones
			status = scheduler.solve_scheduling_ilp(base_path, example_name)
